...
def make_grid(patch_size: int):
    offset = 1 / (2 * patch_size)
    space = np.linspace(-0.5 + offset, 0.5 - offset, patch_size)
    return np.stack(np.meshgrid(space, space, indexing='ij'), axis=-1)  # [y, x]
...
def interpolate_grid(coords, grid, order=0):
    ...
    coords = coords.transpose((0, 3, 1, 2)) * size + (size - 1) / 2
